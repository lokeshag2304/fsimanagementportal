...
export function StatsRow({ stats, className }: StatsRowProps) {
  ...
  return (
    <div
      className={cn(
        "grid gap-3 sm:gap-6 animate-fade-in",
      ...
      {stats.map((stat, index) => (
        ...
        >
          ...
          >
            <path
              ...
              className="
                ...
                /* LIGHT MODE */
                fill-[rgba(255,255,255,var(--glass-opacity))]
                stroke-[rgba(0,0,0,0.12)]
                group-hover:stroke-[rgba(0,0,0,0.18)]

                /* DARK MODE */
                dark:fill-[rgba(255,255,255,var(--glass-opacity))]
                dark:stroke-[rgba(255,255,255,var(--glass-border-opacity))]
                dark:group-hover:stroke-[rgba(255,255,255,calc(var(--glass-border-opacity)+0.1))]
              "
            />
          </svg>

          <button
            className="absolute top-[1px] right-[1px] z-10 w-10 h-10 rounded-full glass-button flex items-center justify-center"
            onClick={() => handlepush(stat.url)}
          >
          ...
          <div className="relative z-[1] p-6 pr-14">
            ...
            <p className="text-[36px] font-bold text-[var(--text-primary)] mb-5">
              {stat.value}
            </p>
              ...
              <div className="flex gap-4 sm:gap-6 lg:gap-10 pt-4 border-t border-[rgba(255,255,255,var(--glass-border-opacity))]">
                {stat.subStats.map((subStat, subIndex) => (
                  <div key={subIndex} className="min-w-0">
                    <p className="text-[var(--text-muted)] text-[10px] sm:text-[11px] mb-1 truncate">
                      {subStat.label}
                    </p>
                    <p className="text-[var(--text-primary)] font-semibold text-sm sm:text-base">
                      {subStat.value}
                    </p>
                  </div>
                ))}
