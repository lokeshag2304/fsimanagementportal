...
interface DomainRecord {
  ...
  today_date: string
  domain_protected: "0" | "1" // Changed from 0|1 to "0"|"1"
  status: 0 | 1 | 2 // Updated to include 2
  remarks: string;
  ...
  quantity?: number | null // Added from API response
  bill_type?: string | null // Added from API response
  start_date?: string | null // Added from API response
  end_date?: string | null // Added from API response
  counter_count?: number | null // Added from API response
  valid_till?: string | null // Added from API response
  updated_at_custom?: string // Added from API response
}
...
export default function DomainsPage() {
  ...
  const statusOptions = [
    ...
    { value: "2", label: "Expired" } // Added option for status 2
  ]
  ...
  const fetchDomainRecords = async () => {
    try {
      ...
      if (response.status) {
        ...
        // Transform API response to match DomainRecord interface
        const transformedData: DomainRecord[] = response.data.map((item: any) => ({
          id: item.id,
          ...
          domain_protected: item.domain_protected?.toString() as "0" | "1" || "0",
                          ...
                          <td colSpan={14} className="py-8 text-center">
                            <div className="flex flex-col items-center justify-center gap-2">
                              ...
                              <span className="text-gray-400">
                                {showDeleted ? "No deleted domain records found" : "No domain records found"}
                              </span>
                              {searchQuery && (
                                <button
